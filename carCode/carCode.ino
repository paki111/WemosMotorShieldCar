...
//Motor shiled I2C default address: 0x30
//PWM frequency: 1000Hz(1kHz)
Motor M1(0x30, _MOTOR_A, 1000, standby_io); //Motor A
Motor M2(0x30, _MOTOR_B, 1000, standby_io); //Motor B
...
void handleJSData(){
  ...
  int aSpeed = abs(y);
  int bSpeed = abs(y);
  //set the speed of each each motor depending on the x-axis
  //it slows down one motor and speeds up the other proportionately 
  aSpeed = constrain(aSpeed + x/2, 0, 100);
  bSpeed = constrain(bSpeed - x/2, 0, 100);
  ...
  if ( y < 0 ){
    M1.setmotor(_CW, aSpeed);
    M2.setmotor(_CW, bSpeed);
  }
  else { 
    M1.setmotor(_CCW, aSpeed);
    M2.setmotor(_CCW, bSpeed);
  }  
  //return an HTTP 200
